...
import com.agilerium.jazzure.config.KafkaProperties;
...
class JazzureKafkaResourceIT {
    ...
    void setup() {
        ...
        JazzureKafkaResource kafkaResource = new JazzureKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jazzure-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jazzure-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
